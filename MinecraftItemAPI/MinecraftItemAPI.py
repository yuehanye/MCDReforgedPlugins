...
class Item:
    ...
    def set_name(self, name: str or r.RTextBase):
        """
        设置物品名称
        ...
        if isinstance(name, str):
            self.data['display']['Name'] = r.RText(name).to_json_str()
        elif isinstance(name, r.RTextBase):
            self.data['display']['Name'] = name.to_json_str()
        return self

    def set_lore(self, *args: str or r.RTextBase):
        """
        设置物品信息
        ...
        for i in args:
            if isinstance(i, str):
                self.data['display']['Lore'].append(r.RText(i).to_json_str())
            elif isinstance(i, r.RTextBase):
    ...
    def set_pages(self, *args: str or r.RTextBase):
        """设置书页"""
        self.data['pages'] = []
        for page in args:
            if self.item_id == 'minecraft:written_book':
                self.data['pages'].append(r.RTextList(page).to_json_str())
            elif self.item_id == 'minecraft:writable_book':
                self.data['pages'].append(r.RTextList(page).to_plain_text())
        return self
...
class Sign(BlockEntity):
    ...
    def set_text(self, line: int, text: str or r.RTextBase):
        """
        设置单行文本
        :param line: 行数 1-4
        ...
        elif isinstance(text, r.RTextBase):
...
class Banner(BlockEntity):
    ...
    def set_custom_name(self, name: str or r.RTextBase):
        """设置名称"""
        if isinstance(name, str):
            self.data['CustomName'] = r.RText(name).to_json_str()
        elif isinstance(name, r.RTextBase):
